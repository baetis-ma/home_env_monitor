...
//reverse bit order of font table byte
...
static int ssd1305_text( char *disp_string )
{
   ...
   //setup and clear framebuffers;
   uint8_t *framebuffer = malloc(len);
   for(int i=0; i<len; i++){ framebuffer[i]= (uint8_t)0x00; }
   uint8_t *framebufferinv = malloc(len);
   for(int i=0; i<len; i++){ framebufferinv[i]= (uint8_t)0x00; }
   ...
   for (int n=0; n< strlen( disp_string); n++) {
      if(disp_string[n] == '|') { framebuf_ptr = 128 * ++line; }
         ...
         for(int a=0; a < 5; a++){
             framebuffer[framebuf_ptr++] = reverse(fonttable5x7[(a + 5*(disp_string[n]-' ' ))%1024]); } 
         framebuf_ptr++; }
          ...
          for(int a=0; a < 5; a++){
             ...
             if(size == 4){ //twice as big
                 f0 = 0; f1 = 0; 
                 ft = reverse(fonttable5x7[a + 5*(disp_string[n]-' ')]) ;
                 if((ft >> 7) & 1) f0 = 0xc0; 
                 ...
                 framebuffer[framebuf_ptr - 1] = f0; 
                 framebuffer[framebuf_ptr - 2] = f0; 
                 framebuffer[128+ (framebuf_ptr - 1)] = f1; 
                 framebuffer[128+ (framebuf_ptr - 2)] = f1; 
             }
          }
       framebuf_ptr++; }
   }

   //invert framebuffer for upsidedown display
   for (int a=1; a <= 1024; a++){
      framebufferinv[1024-a] = framebuffer[a-1];
   }
   ...
   for (int a=0; a < 8; a++){
       vTaskDelay(1); 
       i2c_write( 0x3c,  0x00,  0x40);
       i2c_write_block( 0x3c, 0x40, framebufferinv+a*128, 128);
   }
   free(framebuffer);
   free(framebufferinv);
   return(0);
