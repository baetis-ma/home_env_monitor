...
<img align="right" width="35%" height="450" src="helloworld.png"></img>
##### A system is presented that covers a local area network with remote clients, each having several sensors, distributed over a local area network and a base server. The remote slients, at a periofic rate, collect their data ans report it to the base server. The server reponds with a slot assigngment and the remote stations assigned name. The base server collect all the latest data from all the remote clients as well as it own sensors. Another type for client request from a browser or netcat command from a pc well cause the server to return the data from all of the sensors on the network. A index.html file is shown that displays this data with google visualization gauges and a perl program is described that collects all of the sensor data, time stamps it and stores it in file.
...
9.      items 6-8 will repeat for each registered station.
